...
## removing those with a percentage of 0

data <- data[data$percs != 0,]
...
server <- function(input,output,session){
  ...
  output$map <- plotly::renderPlotly({
    ...
    map_pl <- ggplot(data = map_dat, aes(text = paste("Borough:",NAME))) + 
      geom_sf(aes(fill = percs, color = geography,
                  text = paste0("Borough: ", geography,
                                "<br>",
                                "Count: ", value,
                                "<br>",
                                "Percentage: ", percs, "%"))) + 
